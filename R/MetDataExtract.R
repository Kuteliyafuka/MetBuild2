...
#' @param selected_CpGs character vector of the probeID
...
MetDataExtract <- function(geo_list, selected_CpGs,additional_age_colnames = character()){
  ...
  for (selected_ds in datasets_names){
    ...
    beta_matrix_selectedCpGs <- beta_matrix_rawt[,colnames(beta_matrix_rawt) %in% selected_CpGs]
